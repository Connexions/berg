...
@click.pass_context
def pdf(ctx, collection_path, build, style):
    """Generate a pdf from the baked html in COLLECTION_PATH"""
    input_file = Path(collection_path) / INPUT_FILE
    if not input_file.exists():
        # invoke neb bake if input file is not found
        logger.info('Input file {} not found, running neb bake...'.format(
            INPUT_FILE))
        ctx.invoke(bake, collection_path=collection_path)
    output_file = Path(collection_path) / OUTPUT_FILE
    mounted_input_file = Path(MOUNT_POINT) / INPUT_FILE
