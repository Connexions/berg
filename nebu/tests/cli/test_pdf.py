...
def create_collection_baked_xhtml(datadir):
...
class TestPdfCmd:
    def test_w_collection_baked_xhtml(self, datadir, invoker, pdf_cleanup,
        ...
        with mock.patch('nebu.cli.main.bake') as bake:
            result = invoker(cli, args)
            assert not bake.called
    ...
    def test_wo_collection_baked_xhtml(self, datadir, invoker, pdf_cleanup,
        ...
        from nebu.cli.main import cli
        args = ['pdf', str(datadir)]
        with mock.patch('nebu.cli.pdf.bake') as bake:
            result = invoker(cli, args)
            assert bake.called
            args, kwargs = bake.call_args
